...
function jdn2date(jd) {
    var Z, A, alpha, B, C, D, E, dd, mm, yyyy, F; Z = jd; if (Z < 2299161) { A = Z } else { alpha = INT((Z - 1867216.25) / 36524.25); A = Z + 1 + alpha - INT(alpha / 4) }
...
function decodeLunarYear(yy, k) {
    ...
    if (leapMonth == 0) { for (mm = 1; mm <= 12; mm++) { ly.push(new LunarDate(1, mm, yy, 0, currentJD)); currentJD += regularMonths[mm - 1] } } else {
        for (mm = 1; mm <= leapMonth; mm++) { ly.push(new LunarDate(1, mm, yy, 0, currentJD)); currentJD += regularMonths[mm - 1] }
...
function getYearInfo(yyyy) {
    var yearCode; if (yyyy < 1900) { yearCode = TK19[yyyy - 1800] } else if (yyyy < 2000) { yearCode = TK20[yyyy - 1900] } else if (yyyy < 2100) { yearCode = TK21[yyyy - 2000] } else { yearCode = TK22[yyyy - 2100] }
    return decodeLunarYear(yyyy, yearCode)
}
var FIRST_DAY = jdn(25, 1, 1800); var LAST_DAY = jdn(31, 12, 2199); function findLunarDate(jd, ly) {
...
function getLunarDate(dd, mm, yyyy) {
    var ly, jd; if (yyyy < 1800 || 2199 < yyyy) { }
    ly = getYearInfo(yyyy); jd = jdn(dd, mm, yyyy); if (jd < ly[0].jd) { ly = getYearInfo(yyyy - 1) }
    return findLunarDate(jd, ly)
}
function SunLongitude(jdn) { var T, T2, dr, M, L0, DL, L; T = (jdn - 2451545.0) / 36525; T2 = T * T; dr = PI / 180; M = 357.52910 + 35999.05030 * T - 0.0001559 * T2 - 0.00000048 * T * T2; L0 = 280.46645 + 36000.76983 * T + 0.0003032 * T2; DL = (1.914600 - 0.004817 * T - 0.000014 * T2) * Math.sin(dr * M); DL = DL + (0.019993 - 0.000101 * T) * Math.sin(dr * 2 * M) + 0.000290 * Math.sin(dr * 3 * M); L = L0 + DL; L = L * dr; L = L - PI * 2 * (INT(L / (PI * 2))); return L }
function getSunLongitude(dayNumber, timeZone) { return INT(SunLongitude(dayNumber - 0.5 - timeZone / 24.0) / PI * 12) }
var today = new Date(); var currentLunarDate = getLunarDate(today.getDate(), today.getMonth() + 1, today.getFullYear()); var currentMonth = today.getMonth() + 1; var currentYear = today.getFullYear(); function parseQuery(q) {
    var ret = new Array(); if (q.length < 2) return ret; var s = q.substring(1, q.length); var arr = s.split("&"); var i, j; for (i = 0; i < arr.length; i++) { var a = arr[i].split("="); for (j = 0; j < a.length; j++) { ret.push(a[j]) } }
    return ret
}
...
function getMonth(mm, yy) {
    var ly1, ly2, tet1, jd1, jd2, mm1, yy1, result, i; if (mm < 12) { mm1 = mm + 1; yy1 = yy } else { mm1 = 1; yy1 = yy + 1 }
    jd1 = jdn(1, mm, yy); jd2 = jdn(1, mm1, yy1); ly1 = getYearInfo(yy); tet1 = ly1[0].jd; result = new Array(); if (tet1 <= jd1) { for (i = jd1; i < jd2; i++) { result.push(findLunarDate(i, ly1)) } } else if (jd1 < tet1 && jd2 < tet1) { ly1 = getYearInfo(yy - 1); for (i = jd1; i < jd2; i++) { result.push(findLunarDate(i, ly1)) } } else if (jd1 < tet1 && tet1 <= jd2) {
        ly2 = getYearInfo(yy - 1); for (i = jd1; i < tet1; i++) { result.push(findLunarDate(i, ly2)) }
...
function getDayName(lunarDate) {
    ...
    var cc = getCanChi(lunarDate); var s = "- Ng\u00E0y <strong>" + cc[0] +"</strong>, th\341ng <strong>" + cc[1] + "</strong>, n\u0103m <strong>" + cc[2] + "</strong>"; return s
}
function getYearCanChi(year) { return CAN[(year + 6) % 10] + " " + CHI[(year + 8) % 12] }
function getCanHour0(jdn) { return CAN[(jdn - 1) * 2 % 10] }
function getCanChi(lunar) {
    var dayName, monthName, yearName; dayName = CAN[(lunar.jd + 9) % 10] + " " + CHI[(lunar.jd + 1) % 12]; monthName = CAN[(lunar.year * 12 + lunar.month + 3) % 10] + " " + CHI[(lunar.month + 1) % 12]; if (lunar.leap == 1) { monthName += " (nhu\u1EADn)" }
    yearName = getYearCanChi(lunar.year); return new Array(dayName, monthName, yearName)
}
function getDayString(lunar, solarDay, solarMonth, solarYear) {
    var s; var dayOfWeek = TUAN[(lunar.jd + 1) % 7]; s = "- " + dayOfWeek + " " + solarDay + "/" + solarMonth + "/" + solarYear; s += " -+- "; s = s + "Ng\u00E0y " + lunar.day +" th\341ng " + lunar.month; if (lunar.leap == 1) { s = s + " nhu\u1EADn" }
    return s
}
...
function getGioHoangDao(jd) {
    var chiOfDay = (jd + 1) % 12; var gioHD = GIO_HD[chiOfDay % 6]; var ret = ""; var count = 0; for (var i = 0; i < 12; i++) { if (gioHD.charAt(i) == '1') { ret += CHI[i]; ret += ' (' + (i * 2 + 23) % 24 + '-' + (i * 2 + 1) % 24 + ')'; if (count++ < 5) ret += ', '; if (count == 3) ret += '\n' } }
    return ret
}
var DAYNAMES = new Array("CN", "T2", "T3", "T4", "T5", "T6", "T7"); var PRINT_OPTS = new OutputOptions(); var FONT_SIZES = new Array("9pt", "13pt", "17pt"); var TAB_WIDTHS = new Array("180px", "420px", "600px"); function OutputOptions() { this.fontSize = "13pt"; this.tableWidth = "420px" }
function setOutputSize(size) {
    var idx = 1; if (size == "small") { idx = 0 } else if (size == "big") { idx = 2 } else { idx = 1 }
...
function printYear(yy) {
    var yearName = "N&#x103;m " + getYearCanChi(yy) + " " + yy; var res = ""; res += printStyle(); res += '<table align=center>\n'; res += ('<tr><td colspan="3" class="tennam" onClick="showYearSelect();">' + yearName + '</td></tr>\n'); for (var i = 1; i <= 12; i++) { if (i % 3 == 1) res += '<tr>\n'; res += '<td>\n'; res += printTable(i, yy); res += '</td>\n'; if (i % 3 == 0) res += '</tr>\n' }
...
function printTable(mm, yy) {
    var i, j, k, solar, lunar, cellClass, solarClass, lunarClass; var currentMonth = getMonth(mm, yy); if (currentMonth.length == 0) return; var ld1 = currentMonth[0]; var emptyCells = (ld1.jd + 1) % 7; var MonthHead = mm + "/" + yy; var LunarHead = getYearCanChi(ld1.year); var res = ""; res += ('<table class="thang" style="border: 2px solid #FAACA8" border="2" cellpadding="1" cellspacing="1" width="' + PRINT_OPTS.tableWidth + '">\n'); res += printHead(mm, yy); for (i = 0; i < 6; i++) {
...
function getPrevMonthLink(mm, yy) { var mm1 = mm > 1 ? mm - 1 : 12; var yy1 = mm > 1 ? yy : yy - 1; return '<a style="color: #FAACA8; text-decoration: none;" href="' + window.location.pathname + '?yy=' + yy1 + '&mm=' + mm1 + '">&lt;</a>' }
function getNextMonthLink(mm, yy) { var mm1 = mm < 12 ? mm + 1 : 1; var yy1 = mm < 12 ? yy : yy + 1; return '<a style="color: #FAACA8; text-decoration: none;" href="' + window.location.pathname + '?yy=' + yy1 + '&mm=' + mm1 + '">&gt;</a>' }
function getPrevYearLink(mm, yy) { return '<a style="color: #FAACA8; text-decoration: none;" href="' + window.location.pathname + '?yy=' + (yy - 1) + '&mm=' + mm + '">&lt;&lt;</a>' }
function getNextYearLink(mm, yy) { return '<a style="color: #FAACA8; text-decoration: none;" href="' + window.location.pathname + '?yy=' + (yy + 1) + '&mm=' + mm + '">&gt;&gt;</a>' }
function printHead(mm, yy) {
    var res = ""; var monthName = mm + "/" + yy; res += ('<tr><td colspan="2" class="navi-l">' + getPrevYearLink(mm, yy) + ' &nbsp;' + getPrevMonthLink(mm, yy) + '</td>\n'); res += ('<td colspan="3" class="tenthang" onClick="showMonthSelect();">' + monthName + '</td>\n'); res += ('<td colspan="2" class="navi-r">' + getNextMonthLink(mm, yy) + ' &nbsp;' + getNextYearLink(mm, yy) + '</td></tr>\n'); res += ('<tr onClick="alertAbout();">\n'); for (var i = 0; i <= 6; i++) { res += ('<td class=ngaytuan>' + DAYNAMES[i] + '</td>\n') }
    res += ('<\/tr>\n'); return res
}
function printEmptyCell() { return '<td class=ngaythang><div class=cn>&nbsp;</div> <div class=am>&nbsp;</div></td>\n' }
function printCell(lunarDate, solarDate, solarMonth, solarYear) {
    var cellClass, solarClass, lunarClass, solarColor; cellClass = "ngaythang"; solarClass = "t2t6"; lunarClass = "am"; solarColor = "#"; var dow = (lunarDate.jd + 1) % 7; if (dow == 0) { solarClass = "cn"; solarColor = "red" } else if (dow == 6) { solarClass = "t7"; solarColor = "green" }
    ...
    var res = ""; var args = lunarDate.day + "," + lunarDate.month + "," + lunarDate.year + "," + lunarDate.leap; args += ("," + lunarDate.jd + "," + solarDate + "," + solarMonth + "," + solarYear); res += ('<td class="' + cellClass + '" data-args="' + args + '"'); if (lunarDate != null) res += (' title="' + getDayName(lunarDate) + '" onClick="alertDayInfo(' + args + ');"'); res += (' <div style=color:' + solarColor + ' class="' + solarClass + '">' + solarDate + '</div> <div class="' + lunarClass + '">' + lunar + '</div></td>\n'); return res
}
function printFoot() { var res = ""; res += '<script language="JavaScript" src="amlich-hnd.js"></script>\n'; return res }
function showMonthSelect() { var home = "http://www.ifis.uni-luebeck.de/~duc/amlich/JavaScript/"; window.open(home, "win2702", "menubar=yes,scrollbars=yes,status=yes,toolbar=yes,resizable=yes,location=yes") }
function showYearSelect() { window.print() }
function infoCellSelect(id) { if (id == 0) { } }
function alertDayInfo(dd, mm, yy, leap, jd, sday, smonth, syear) { var lunar = new LunarDate(dd, mm, yy, leap, jd); var s = getDayString(lunar, sday, smonth, syear); s += " \u00E2m l\u1ECBch<br>"; s += getDayName(lunar); s += "<br>- <strong>Gi\u1EDD \u0111\u1EA7u ng\u00E0y</strong>: " + getCanHour0(jd) + " " + CHI[0]; s += "<br>- <strong>Ti\u1EBFt</strong>: " + TIETKHI[getSunLongitude(jd + 1, 7.0)]; s += "<br>- <strong>Gi\u1EDD ho\u00E0ng \u0111\u1EA1o</strong>: " + getGioHoangDao(jd); window.parent.postMessage({ type: 'showContent', content: s }, '*') }
